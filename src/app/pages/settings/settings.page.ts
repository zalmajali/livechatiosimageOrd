...
//page setting
  ...
  public order_chat_one:any;
  public order_chat_tow:any;
  public order_chat_three:any;
  public selectedOption:any;
  public first_dir:any;
  public last_dir:any;
  public chatAssignment:any;
  public email:any;
  public mobile:any;
  ...
  async initialiseTranslation(){
   ...
   this.translate.get('yes').subscribe((res: string) => {
    ...
    this.translate.get('order_chat_one').subscribe((res: string) => {
      this.order_chat_one = res;
    });
    this.translate.get('order_chat_tow').subscribe((res: string) => {
      this.order_chat_tow = res;
    });
    this.translate.get('order_chat_three').subscribe((res: string) => {
      this.order_chat_three = res;
    });
  ...
  async ngOnInit() {
    ...
    await this.functiongetTeamUsers();
    await this.functionGetSettings();
  }
  async functionGetSettings(){
    ...
    this.userService.chatGetSetting(sendValues).then(async data=>{
      this.returnResultTeamData = data;
      let errorData = this.returnResultTeamData.messageId;
      if(errorData == "OK"){
        this.chatAssignment = this.returnResultTeamData.chatAssignment;
        this.email = this.returnResultTeamData.email;
        this.mobile = this.returnResultTeamData.mobile;
        this.selectedOption = this.returnResultTeamData.orderList;
        this.nameOfUser = this.returnResultTeamData.name;
        this.startMessage = this.returnResultTeamData.startMessage;
      }
    }).catch(error=>{
      this.functionGetSettings()
    });
  }
  async changeOrder(event:any){
    ...
    this.apiKey = md5Hash.finalize();
    this.apiKey=this.apiKey.toString();
    let sendValues = {'mainUserName':this.mainUserName,'userName':this.userName,'password':this.password,'apiKey':this.apiKey,'chatAssignment':this.chatAssignment,'newPassword':this.password,'email':this.email,'name':this.name,'mobile':this.mobile,'startMessage':this.startMessage,'orderList':this.selectedOption};
    this.userService.chatSetSetting(sendValues).then(async data=>{
  ...
  async functionLogOut(){
      ...
      buttons: [
        {
          ...
          cssClass: 'alertButton',
          handler: async () => {
            await this.checkLogOutDataUser();
            this.storage.remove('mainUserName');
            this.storage.remove('userName');
            this.storage.remove('password');
            this.storage.remove('apiKey');
    ...
    async checkLogOutDataUser(){
    this.userName = await this.storage.get('userName');
    let sendValues = {'userName':this.userName};
     this.userService.loginOutDataUser(sendValues).then(async data=>{
    }).catch(error=>{
      this.checkLogOutDataUser();
    });
  }
  functionSendEmail() {
